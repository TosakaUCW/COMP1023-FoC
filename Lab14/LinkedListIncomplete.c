...
Node* createList(char fileName[])
{
	...
	while (fscanf(fp, "%s%d", stuRecord.name, &stuRecord.score) != EOF){
		...
		strcpy(_________________);   //copy name to new node
		pNewRecord -> score = ________________; //assign score to new node
...
Node* insertARecord(Node *pHead)
{
	...
	while(pNode1 != NULL && pNode1 -> score > pNewRecord -> score){
		...
		pNode2 = __________; //record the current node before pNode1 moves to point to the next
		pNode1 = __________;
	}
    
	if (pNode1 == pHead){ //for the case pHead -> score < pNewRecord -> score
		pNewRecord -> next = __________;
		pHead = ___________;
	}
	else {//for the cases pNode1 == NULL or pNode1 -> score < pNewRecord -> score < pNode2 -> score
		___________ -> next = pNewRecord;
	    pNewRecord -> next = ______________;	
	}
...
Node* deleteARecord(Node *pHead)
{
	...
	scanf("%s", name);
    while(pNode1 != _____________ && ______________________){
		pNode2 = pNode1;
		pNode1 = pNode1 -> next;
	}
	if (pNode1 == NULL)
	...
	if (pNode1 == pHead) //for the case that head's name is name
	    pHead = ____________; //the second node will be the head
	else //pNode1 -> name is name
		pNode2 -> next = ________________;
    
	free(_________); //free the memory for original head
	return pHead;
}
Node* saveLinkedList(Node* pHead, char fileName[20])
{
